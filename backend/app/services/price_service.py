...
class PriceCalculator:
    ...
    async def get_usd_try_rate(self) -> Decimal:
        """
        Get current USD/TRY exchange rate from TCMB (Turkish Central Bank).
        Falls back to 50.00 if API fails.
        """
        try:
            # TCMB EVDS API (requires API key for production)
            # For now, using a fallback rate
            # TODO: Implement TCMB API integration
            return Decimal("50.00")
        except Exception as e:
            print(f"USD rate fetch error: {e}")
            return Decimal("50.00")  # Fallback rate
